...
const ChatBot = () => {
  ...
  return (
    <div className="fixed bottom-20 right-5 z-50 flex flex-col items-end">
      {/* Speech bubble */}
      {!isOpen && <ChatBubble show={showBubble} />}
      ...
      {isOpen && (
        <div className={cn(
          "bg-white rounded-lg shadow-xl flex flex-col border border-gray-200 overflow-hidden transition-all duration-300",
          isFullscreen 
            ? "fixed right-4 top-20 bottom-4 w-72 rounded-lg" 
            : "w-72 h-[400px]"
        )}>
          {/* Header */}
