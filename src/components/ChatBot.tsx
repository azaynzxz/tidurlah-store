    ...
    if (collectingName) {
      ...
      setTimeout(() => {
        ...
        setMessages([
          ...updatedMessages,
          { 
            text: "Dari instansi/kampus/perusahaan mana? (Opsional - bisa diisi 'Tidak ada' jika tidak memiliki):", 
            isBot: true,
            showNameForm: true 
          }
        ]);
  ...
  const handleSuggestionClick = (keyword: string) => {
    ...
    if (keyword === "chat with admin") {
      if (!userName) {
        ...
        setMessages([
          ...messages,
          { text: "Siapa nama Anda?", isBot: false },
          { 
            ...
            {messages.map((msg, idx) => (
              ...
              >
                ...
                {msg.isWhatsAppPrompt && (() => {
                  ...
                  let whatsappMessage = "";
                  if (msg.collectedName) {
                    if (msg.collectedInstansi && msg.collectedInstansi.toLowerCase() !== 'tidak ada' && msg.collectedInstansi.trim() !== '') {
                      whatsappMessage = `Halo Mincard, saya ${msg.collectedName} dari ${msg.collectedInstansi} ingin bertanya terkait produk tidurlah.com`;
                    } else {
                      whatsappMessage = `Halo Mincard, saya ${msg.collectedName} ingin bertanya terkait produk tidurlah.com`;
                    }
                  } else {
